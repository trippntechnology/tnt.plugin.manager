...
public class Manager
{
    ...
    /// <summary>
    /// Loads and registers all <see cref="Plugin"/> instances found in the specified directory.
    /// Each valid plugin assembly (.dll) is loaded, instantiated, and merged into the application's UI.
    /// </summary>
    /// <param name="pluginDirectory">The directory path where plugin assemblies are located.</param>
    ...
    public void Register(string pluginDirectory)
    {
        ...
        foreach (string file in files)
        {
            ...
            try
            {
                types = Utilities.Utilities.GetTypes(assembly, t => HasBaseType(t, typeof(Plugin)) && !t.IsAbstract);
            }
            catch { }
